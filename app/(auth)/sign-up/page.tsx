...
export default function SignUpPage() {
  ...
  const handleVerify = async () => {
    ...
    // New users go to onboarding; an already-registered number lands home.
          ...
          <AnimatePresence mode="wait">
            ...
            {step === "otp" && (
              <motion.div key="otp" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}>
                ...
                <button onClick={handleSend} disabled={sending} className="mt-4 w-full text-center text-[12px] text-gold/60 hover:text-gold transition-colors py-1 disabled:opacity-50">
                  {t("auth.resend")}
                </button>
              </motion.div>
            )}
