...
export const api = {
  ...
  /**
   * Current user's personalized horoscope. Daily is generated by the nightly
   * cron job — throws ApiError(404) when it hasn't run yet for today
   * (callers should treat that as an empty state, not a hard error).
   * Weekly/monthly/yearly are generated on first request per period and
   * cached, so they never 404 (may just be slower on the very first call).
   */
  horoscope: (period: PersonalizedHoroscopePeriod = "daily") =>
    request<PersonalizedHoroscope>(`/v1/horoscope?period=${period}`, { auth: true }),
